...
namespace SteamPulse
{
    public partial class DLCBrowser : Form
    {
        ...
        {
            IRTCheck = UserSettings.CheckIRT;
            if (IRTCheck == true)
            {
            ...
            if (UserSettings.LoadDLCImage == true)
            {
                image.Visible = true;
            }
            else
        ...
        private void BackgroundWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            {
                limit = UserSettings.DefualtDlcCount;
            }
            try
            {
                ...
                foreach (string appid in ID)
                {
                    ...
                    if (UserSettings.DeveloperMode == true)
                    {
                        if (UserSettings.LoadDLCImage == false)
                        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                DataTable.Invoke((MethodInvoker)(() => row.Cells["price"].Value = string.Format("{0} {1} - {2}%", string.Format("{0:n0}", LoadData.Store.DLC.Data.Price), UserSettings.Currency.Unit, LoadData.Store.DLC.Data.Discount)));
                            }
                            else
                            {
                                DataTable.Invoke((MethodInvoker)(() => row.Cells["price"].Value = string.Format("{0} {1}", string.Format("{0:n0}", LoadData.Store.DLC.Data.Price), UserSettings.Currency.Unit)));
                            }
                ...
                else if (sum < LoadData.Market.Ticket.LowestSellOrderNoFee)
                {
                    if (UserSettings.CheckIRT == true)
                    {
                        ...
                        if (TicketIRT != 0)
                        {
                            LabelSum.Invoke((MethodInvoker)(() => LabelSum.Text = string.Format("Sum: {0} {1} - {2} Ticket - {3}IRT", sum, UserSettings.Currency.Unit, (int)Math.Ceiling(Math.Round(sum / LoadData.Market.Ticket.LowestSellOrderNoFee, 1)), string.Format("{0:n0} ", (int)Math.Ceiling(Math.Round(sum / LoadData.Market.Ticket.LowestSellOrderNoFee, 1)) * TicketIRT))));
                        }
                        else
                        {
                            LabelSum.Invoke((MethodInvoker)(() => LabelSum.Text = string.Format("Sum: {0} {1} - {2} Ticket", sum, UserSettings.Currency.Unit, (int)Math.Ceiling(Math.Round(sum / LoadData.Market.Ticket.LowestSellOrderNoFee, 1)))));
                        }
                    }
                    else
                    {
                ...
                else
                {
                    if (UserSettings.CheckIRT == true)
                    {
                        if (KeyIRT != 0)
                        {
                            LabelSum.Invoke((MethodInvoker)(() => LabelSum.Text = string.Format("Sum: {0} {1} - {2} Key - {3}IRT", sum, UserSettings.Currency.Unit, (int)Math.Ceiling(Math.Round(sum / LoadData.Market.Key.LowestSellOrderNoFee, 1)), string.Format("{0:n0} ", (int)Math.Ceiling(Math.Round(sum / LoadData.Market.Key.LowestSellOrderNoFee, 1)) * KeyIRT))));
                        }
                        else
                        {
                            LabelSum.Invoke((MethodInvoker)(() => LabelSum.Text = string.Format("Sum: {0} {1} - {2} Key", sum, UserSettings.Currency.Unit, (int)Math.Ceiling(Math.Round(sum / LoadData.Market.Key.LowestSellOrderNoFee, 1)))));
                        }
                    }
                    else
                    {
